...
var Installer = function(callback) {
  if (!(this instanceof Installer)) return new Installer(callback);

  var self = this;

  var installCache = {};

  Glob('myriad/cache/*', { strict: true }, function(err, entries) {
    if (err) {
      callback(err);
    }
    else {
      entries.forEach(function(entry) {
        installCache[Path.basename(entry)] = {
          installed: true
        };
      });

      self._installCache = installCache;
      callback();
    }
  });
};
  ...
  Async.waterfall(
    [
      ...
      function(packageTempDir, packageJson, packageData, callback) {
        ...
        var install = self._installCache[packageId];
        ...
        if (install) {
          ...
          self._installCache[packageId] = install;
          ...
          Async.waterfall(
            ...
            function(err) {
              if (err) {
                ...
                install.waiting.forEach(function(waitingCallback) {
                  waitingCallback(null, packageId, cachePackageDir);
                });
              }
            }
          );
