...
Spawner.spawn = function(options, driver) {
  function send(options) {
    Debug('Sending ' + options.event + ' message');
    driver.text(JSON.stringify(options));
  }
  ...
  Async.waterfall(
    ...
    function(err, packageDir) {
      if (err) {
        ...
        this._child = WinSpawn(options.bin, options.args, {
          cwd: packageDir,
          stdio: 'pipe',
          env: options.env
        });
        ...
        this._child.stdout.on('data', function(data) {
          send({ event: 'child_stdout', data: data.toString('base64') });
        });

        this._child.stderr.on('data', function(data) {
          send({ event: 'child_stderr', data: data.toString('base64') });
        });

        this._child.on('error', function(err) {
          send({ event: 'child_error', error: err })
        });

        this._child.on('exit', function(code, signal) {
          send({ event: 'child_exit', code: code, signal: signal });
        });

        this._child.on('close', function(code, signal) {
          send({ event: 'child_close', code: code, signal: signal });
        });

        this._child.on('disconnect', function() {
          send({ event: 'child_disconnect' });
        });

//        this._child.on('message', function(message) {
//          send({ event: 'child_message', data: message.toString('base64') });
//        });
      }
    }
  );
